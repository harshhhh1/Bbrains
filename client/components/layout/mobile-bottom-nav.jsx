...
export function MobileBottomNav({ user }) {
  ...
  const userRoles =
    user?.roles || [user?.appRole || user?.type || "student"].filter(Boolean);
  const resolvedRoles = resolveRole(userRoles);
        ...
        <div className="flex-1 flex items-center overflow-x-auto scrollbar-hide px-2">
          {navItems.map((item) => {
            ...
            const isActive =
              pathname === item.url ||
              (item.url !== "/dashboard" &&
                pathname.startsWith(String(item.url)));
            const Icon =
              item.icon || LucideIcons.Circle || LucideIcons.HelpCircle;
              ...
              return (
                ...
                >
                  ...
                  <DropdownMenuContent
                    align="center"
                    side="top"
                    className="w-48 mb-2 rounded-2xl p-2 animate-in slide-in-from-bottom-2"
                  >
                    {item.subItems?.map((sub) => {
                      const SubIcon = sub.icon || LucideIcons.Circle;
                      return (
                        <DropdownMenuItem
                          key={sub.url}
