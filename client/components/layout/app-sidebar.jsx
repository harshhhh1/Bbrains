...
const UserProfileCard = dynamic(
  () =>
    import("@/components/user-profile-card").then((mod) => mod.UserProfileCard),
  {
    ssr: true,
  },
);

export function AppSidebar({ user, sidebarAccessOverride }) {
  const pathname = usePathname();
  const { state } = useSidebar();
  const [mounted, setMounted] = useState(false);
  const isCollapsed = state === "collapsed";
  const { chatUnreadTotal, assignmentUnreadTotal, productUnreadTotal } =
    useNotifications();
  ...
  const groups = React.useMemo(
    () => getSidebarGroups(resolvedRoles, sidebarAccessOverride),
    [resolvedRoles, sidebarAccessOverride],
  );
  ...
  return (
    <Sidebar collapsible="icon" className="border-none">
      ...
      <SidebarContent
        className={`bg-sidebar ${isCollapsed ? "px-1.5" : "px-3"}`}
      >
        {groups.map((group, groupIndex) => (
          <SidebarGroup key={groupIndex}>
            ...
            <SidebarGroupContent>
              <SidebarMenu
                className={`${isCollapsed ? "space-y-2" : "space-y-1.5"}`}
              >
                {group.items.map((item) => {
                  ...
                  const showDot =
                    ...
                    (isAssignments && assignmentUnreadTotal > 0) ||
                    (isProducts && productUnreadTotal > 0);
                  const showBadge = showDot && !isCollapsed;
                  const badgeCount = isChat
                    ? chatUnreadTotal
                    : isAssignments
                      ? assignmentUnreadTotal
                      : productUnreadTotal;
                        ...
                        <SidebarMenuButton asChild tooltip={item.title}>
                          ...
                          >
                            <div className="relative">
                              <item.icon
                                className={`${isCollapsed ? "h-5.5 w-5.5" : "h-5 w-5"} shrink-0 ${isActive ? "text-white" : ""}`}
                              />
                              {showDot && isCollapsed && (
                                <span className="absolute -top-1 -right-1 h-3 w-3 bg-red-500 rounded-full border-2 border-white dark:border-gray-800" />
                              )}
                            </div>
                            <span className="text-[13px] group-data-[collapsible=icon]:hidden flex-1">
                              {item.title}
                            </span>
                          </Link>
                        </SidebarMenuButton>
        ...
        {user && (
          <div className="flex flex-col gap-4">
            ...
            <div className={isCollapsed ? "flex justify-center" : "px-2"}>
              {!isCollapsed && (
                <h3 className="text-[10px] font-bold text-sidebar-foreground/60 uppercase tracking-[0.1em] mb-4">
                  Account
                </h3>
              )}
              <button
              ...
              >
                <div className="relative">
                  ...
                  <div
                    className={`absolute -bottom-0.5 -right-0.5 bg-green-500 w-3 h-3 rounded-full border-2 border-white dark:border-gray-800`}
                  />
                </div>
                {!isCollapsed && (
