...
export function ChatImagePreview({
  ...
  const [messageAspectRatio, setMessageAspectRatio] = useState(null);
  ...
  const handleDownload = async (e) => {
    ...
    try {
      ...
      if (attachment.url.includes("cloudinary.com")) {
        ...
        const cleanBaseName = (attachment.name || "file")
          .split(".")[0]
          .replace(/[^\w-]/g, "_");
  ...
  return (
    <Dialog>
      <DialogTrigger asChild>
        <div
          className={cn(
            "relative group cursor-pointer overflow-hidden rounded-xl border border-border bg-muted/30 shadow-sm hover:shadow-md transition-all duration-200",
            className,
          )}
        >
          {isImage ? (
            <div
              ...
              style={
                variant === "message"
                  ? { aspectRatio: messageAspectRatio ?? 16 / 9 }
                  : undefined
              }
            >
              <Image
