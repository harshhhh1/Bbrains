...
export default function AttendancePage() {
  ...
  const fetchStudentsAndAttendance = useCallback(async () => {
    try {
      ...
      const classTeacherCourse =
        profileRes.data.data?.classTeacherCourse ?? null;
      ...
      const [studentsRes, attendanceRes] = await Promise.all([
        client.get(`/courses/${classTeacherCourse.id}/students`),
        attendanceApi.getAttendanceByDate(formattedDate),
      ]);

      const allStudents = (studentsRes.data.data || []).map(({ user }) => ({
        ...user,
      }));
  ...
  const handleMarkAttendance = async (studentId, status, notes) => {
    ...
    try {
      ...
      setStudents((prev) =>
        prev.map((s) => (s.id === studentId ? { ...s, isUpdating: false } : s)),
      );
    }
  };
  ...
  const markAllPresent = async () => {
    ...
    // Optimistic Update
    setStudents((prev) =>
      prev.map((s) => ({ ...s, currentStatus: "present", isUpdating: true })),
    );

    const request = attendanceApi
      .markAttendanceBulk({
    ...
    <div className="mx-auto w-full max-w-7xl p-6 md:p-12 space-y-12">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <SectionHeader
          ...
          subtitle={
            hasAssignedClass
              ? `Daily records for ${assignedCourse?.name}.`
              : "Attendance list (Awaiting assignment)."
          }
        />
        ...
        <div className="flex flex-col sm:flex-row items-center gap-3">
          <Popover>
            <PopoverTrigger asChild>
              <Button
                variant="outline"
                className={cn(
                  "h-14 min-w-[240px] rounded-2xl font-bold justify-start",
            ...
            <PopoverContent
              className="w-auto p-0 rounded-2xl overflow-hidden border-border/60 shadow-2xl"
              align="end"
            >
              <Calendar
                mode="single"
          ...
          <Button
            ...
            disabled={loading || !hasAssignedClass || students.length === 0}
        ...
        <div className="flex items-center justify-between">
          <div className="space-y-1">
            ...
            <p className="text-muted-foreground font-medium">
              Student list for current academic cycle.
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 gap-4">
            {filteredStudents.map((student) => (
              <AttendanceStudentCard
                ...
                student={student}
                onMark={(status, notes) =>
                  handleMarkAttendance(student.id, status, notes)
                }
                onNotesChange={(val) =>
                  setStudents((prev) =>
                    prev.map((s) =>
