...
export function ProductsApprovals() {
  ...
  const load = useCallback(async () => {
    try {
      ...
      const res = await c.get("/market/pending");
  ...
  return (
    <div className="space-y-6">
      <SectionHeader
        ...
        subtitle={`${products.length} pending review`}
      />
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {products.map((product) => {
            ...
            return (
              <Card
                key={product.id}
                className="group overflow-hidden rounded-[1.5rem] border-border/40 shadow-sm hover:shadow-md transition-all duration-300 flex flex-col"
              >
                  ...
                  <div className="flex-1 overflow-y-auto p-6 space-y-6">
                    <div>
                      <div className="flex items-center gap-2 mb-2">
                        ...
                        {metadata.category && (
                          <Badge
                            variant="secondary"
                            className="text-[10px] uppercase font-bold tracking-widest bg-muted/50"
                          >
                            {metadata.category}
                          </Badge>
                        )}
                      ...
                      <p className="text-sm font-medium text-muted-foreground">
                        ...
                        <span className="text-brand-purple font-bold">
                          @{selectedProduct.creator?.username}
                        </span>
                      </p>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="bg-muted/30 rounded-2xl p-4 border border-border/30">
                        ...
                        <span className="block text-xl font-black text-brand-orange">
                          {fmtCurrency(selectedProduct.price)}
                        </span>
                      </div>
                      <div className="bg-muted/30 rounded-2xl p-4 border border-border/30">
            ...
            <DrawerHeader className="border-b border-border/60 p-6 text-left">
              <div className="flex items-start justify-between gap-4">
                <div className="space-y-2">
                  <DrawerTitle className="text-xl font-black text-red-600">
                    Reject Product
                  </DrawerTitle>
