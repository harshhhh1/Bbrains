...
export const SubjectSelector = ({
  ...
  <div className="col-span-2 space-y-2">
    <label className="text-xs font-bold uppercase tracking-wider text-muted-foreground">
      Assign Teaching Subjects (Multi-select)
    </label>
    <div className="grid grid-cols-2 gap-2 bg-black/20 p-4 rounded-xl border border-white/5 max-h-64 overflow-y-auto">
      {subjects.map((subject) => (
        <label
          key={subject.name}
          className="flex items-center gap-3 cursor-pointer group hover:bg-white/5 p-3 rounded-lg transition-colors border border-transparent hover:border-white/5"
        >
          <input
            type="checkbox"
