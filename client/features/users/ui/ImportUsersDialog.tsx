      ...
      <div className="relative bg-[#111111] border border-white/10 rounded-2xl shadow-2xl p-8 max-w-4xl w-full max-h-[90vh] overflow-y-auto animate-in fade-in zoom-in duration-300 text-white shadow-primary/10">
        ...
        {submitting && (
          <div className="space-y-5 p-8 bg-white/5 rounded-3xl border border-white/10 relative overflow-hidden">
            ...
            <div className="flex justify-between items-center">
              ...
              <span className="text-3xl font-black text-primary tabular-nums">{progress}%</span>
            </div>
        ...
        {result && !submitting && (
          <div className={`p-8 rounded-3xl border flex items-start gap-6 shadow-[0_20px_50px_rgba(0,0,0,0.5)] animate-in slide-in-from-bottom-8 fade-in-0 duration-700 ${
            result.error ? 'bg-red-500/[0.08] border-red-500/30' : 'bg-emerald-500/[0.08] border-emerald-500/30'
          }`}>
            <div className={`mt-1 rounded-2xl p-3 flex-shrink-0 shadow-lg ${result.error ? 'bg-red-500/20 text-red-500 shadow-red-500/10' : 'bg-emerald-500/20 text-emerald-500 shadow-emerald-500/10'}`}>
              {result.error ? (
            ...
            <div className="flex-1">
              <p className={`font-black text-xl tracking-tight ${result.error ? 'text-red-400' : 'text-emerald-400'}`}>
                {result.error ? 'Import Halted' : 'Batch Protocol Successful'}
              </p>
              <p className={`mt-2 font-medium leading-relaxed ${result.error ? 'text-red-200/60' : 'text-emerald-200/60'}`}>
                {result.error 
