...
export default function UsersPageClient() {
  ...
  useEffect(() => {
    ...
    if (pageCollegeId) {
      import("@/services/api/base").then((m) =>
        m.setImpersonateCollegeId(pageCollegeId),
      );
    } else {
      import("@/services/api/base").then((m) =>
        m.setImpersonateCollegeId(null),
      );
    }
  }, [pageCollegeId]);
  ...
  return (
    <div className="mx-auto w-full max-w-7xl p-6 md:p-12 space-y-10">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground mt-1">
            Create accounts and manage custom role assignments in one place
          </p>
        </div>
      </div>
      ...
      <CrudDrawer
        ...
        title={
          editingUser
            ? `Edit ${editingUser.userDetails?.firstName}'s Profile`
            : `Add ${form.type.charAt(0).toUpperCase() + form.type.slice(1)}`
        }
        description={
          editingUser
            ? "Update user details and assigned subjects."
            : `Create a new ${form.type} account with custom permissions.`
        }
