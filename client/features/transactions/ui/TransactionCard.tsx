...
import { formatCurrency, formatDate, formatCategory, downloadReceipt } from "@/features/transactions/model/utils"
        ...
        <div className="flex items-center gap-4 w-full sm:w-auto">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-1">
              <span className="font-bold text-base">{formatCategory(transaction.category)}</span>
              <Badge
                variant={transaction.status?.toLowerCase() === "success" ? "outline" : "destructive"}
                className={cn(
            ...
            <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
              <span className="flex items-center gap-1 font-medium">
                <Calendar className="size-3" /> {formatDate(transaction.transactionDate)}
              </span>
              ...
              <span className="flex items-center gap-1 font-medium font-mono truncate max-w-[200px]">
                <Hash className="size-3" /> {transaction.referenceId || "No reference"}
              </span>
            </div>
          </div>
        </div>
