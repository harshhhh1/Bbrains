...
import { AnnouncementItem } from "@/features/announcements/ui/AnnouncementItem";
import { PostEditor } from "@/features/announcements/ui/PostEditor";
import { AcknowledgeDrawer } from "@/features/announcements/ui/AcknowledgeDrawer";

export function AnnouncementsContent({ initialAnnouncements, currentUser }) {
  ...
  const filteredAnnouncements = announcements.filter((a) => {
    const titleMatch = (a.title?.toLowerCase() || "").includes(
      searchQuery.toLowerCase(),
    );
    const descMatch = (a.description?.toLowerCase() || "").includes(
      searchQuery.toLowerCase(),
    );
    ...
    const matchesCollege =
      a.isGlobal ||
      !a.collegeId ||
      String(a.collegeId) === String(currentCollegeId);
    ...
    try {
      ...
      if (response.success && response.data) {
        setAnnouncements((prev) =>
          prev.map((a) => (String(a.id) === String(id) ? response.data : a)),
        );
  ...
  const handleViewAcknowledged = async (id) => {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (response.success) {
        setAnnouncements((prev) =>
          prev.filter((a) => String(a.id) !== announcementToDelete),
        );
      ...
      >
        <header className="mb-10">
          ...
          <p className="text-muted-foreground mt-2 text-lg font-medium">
            Internal news and faculty broadcasts.
          </p>
        </header>
          ...
          {filteredAnnouncements.length > 0 ? (
            ...
            <div className="py-20 text-center rounded-3xl border-2 border-dashed border-border/40 bg-muted/10">
              ...
              <p className="text-muted-foreground mt-2">
                No broadcasts matching your criteria.
              </p>
            </div>
          )}
          ...
          {hasMoreAnnouncements && (
            <div className="flex justify-center pt-10">
              <Button
                variant="outline"
                onClick={() =>
                  setVisibleCount((c) => c + ANNOUNCEMENTS_PAGE_SIZE)
                }
        ...
        <DrawerContent className="p-0 data-[vaul-drawer-direction=right]:w-full data-[vaul-drawer-direction=right]:sm:max-w-md before:inset-0 before:rounded-none before:border-white/10 before:bg-background sm:p-0 sm:before:rounded-l-[2rem]">
          <div className="flex h-[100dvh] max-h-[100dvh] flex-col overflow-hidden">
            <DrawerHeader className="p-6 text-left">
              <DrawerTitle className="text-2xl font-black text-destructive">
                Wipe Record
              </DrawerTitle>
              <DrawerDescription className="text-muted-foreground font-medium">
                This action will permanently purge this announcement from the
                list.
              </DrawerDescription>
            </DrawerHeader>
