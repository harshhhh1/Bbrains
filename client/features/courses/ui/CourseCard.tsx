...
import { BookOpen, Users, Plus, CheckCircle2, GraduationCap, LayoutGrid } from "lucide-react";
...
export function CourseCard({ course, enrolling, onEnroll }: CourseCardProps) {
  ...
  const courseStandard = 
    (course as any).standard || 
    (course as any).grade || 
    (course as any).classLevel || 
    (course as any).metadata?.standard || 
    (course as any).metadata?.grade ||
    null;
  ...
  return (
    <Card className="hover:shadow-2xl transition-all duration-500 border-border/60 bg-card/50 backdrop-blur-md rounded-[2.5rem] overflow-hidden group flex flex-col h-full">
      <CardContent className="p-8 flex-1 flex flex-col">
        ...
        <div className="flex items-start justify-between mb-8">
          ...
          <div className="flex flex-col items-end gap-2">
            {course.isEnrolled ? (
              ...
              <Badge variant="outline" className="px-3 py-1 font-black uppercase tracking-widest text-[9px] h-7 rounded-lg">
                Available Access
              </Badge>
            )}
            {courseStandard ? (
              <div className="flex items-center gap-2 px-3 py-1.5 bg-muted/40 rounded-xl border border-border/50 shadow-sm animate-in fade-in zoom-in-95 duration-500">
                ...
                <span className="text-xs font-black text-foreground tracking-tight uppercase">{courseStandard}</span>
              </div>
            ) : (
              <div className="flex items-center gap-2 px-3 py-1.5 bg-red-500/5 rounded-xl border border-red-500/10 opacity-60">
                 ...
                 <span className="text-[10px] font-black uppercase tracking-tighter text-red-500/70">Standard Pending</span>
              </div>
            )}
          </div>
        </div>
        ...
        {/* Title & Info */}
        <div className="mb-8">
          <h3 className="font-black text-2xl text-foreground leading-[1.1] tracking-tighter group-hover:text-primary transition-colors">{course.name}</h3>
          {course.description && (
            <p className="text-sm text-muted-foreground mt-4 line-clamp-2 leading-relaxed font-medium opacity-80">{course.description}</p>
          )}
        </div>
          ...
          <div className="mb-10 space-y-5 flex-1">
            <div className="flex items-center justify-between px-1">
               <div className="flex items-center gap-2">
                  ...
                  <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-foreground/70">Syllabus Progress</h4>
               </div>
               <span className="text-[10px] font-black text-primary bg-primary/10 px-2 py-0.5 rounded-lg">{progressEntries.length} Subjects</span>
            </div>
            ...
            {progressEntries.length > 0 ? (
              <div className="grid grid-cols-2 gap-3">
                {progressEntries.map((entry) => {
                  ...
                  return (
                    <div key={`${course.id}-${entry.subject}`} className={cn(
                      "p-4 rounded-[1.5rem] border transition-all hover:scale-[1.02] relative group/item",
                      isDone ? "bg-emerald-500/5 border-emerald-500/20" : "bg-card border-border/60 shadow-sm hover:border-primary/30"
                    )}>
                      <div className="flex flex-col gap-2">
                         <span className="truncate text-[10px] font-black text-foreground/80 tracking-tight uppercase leading-none">{entry.subject}</span>
                         <div className="flex items-baseline gap-1">
                            <span className="text-lg font-black tabular-nums tracking-tighter leading-none">{entry.completedChapters}</span>
                            <span className="text-[9px] font-bold text-muted-foreground/40 uppercase">/ {entry.totalChapters || "∞"}</span>
                         </div>
                      </div>
                      <div className="mt-3 relative">
                        <Progress value={percent} className="h-1 rounded-full bg-muted/40" />
                        {isDone && (
          ...
          <div className="flex items-center gap-3">
            <Avatar className="w-10 h-10 border-2 border-background shadow-md ring-1 ring-border/20">
              <AvatarFallback className="bg-primary/10 text-primary text-xs font-black">
                {getInitials(course.classTeacher?.userDetails?.firstName || course.classTeacher?.username)}
              </AvatarFallback>
            </Avatar>
            <div>
              <p className="text-[9px] font-black uppercase text-muted-foreground tracking-widest opacity-60 mb-0.5">Faculty Lead</p>
              <p className="text-xs font-bold text-foreground">
                {course.classTeacher?.userDetails?.firstName || course.classTeacher?.username || "Agent TBA"}
              </p>
            </div>
          </div>
          <div className="text-right">
             <p className="text-[9px] font-black uppercase text-muted-foreground tracking-widest opacity-60 mb-0.5">Enrolled</p>
             <div className="flex items-center gap-1.5 justify-end text-sm font-black text-foreground tabular-nums">
                <Users className="w-4 h-4 text-primary" /> {course.enrolledStudents || 0}
             </div>
          </div>
        </div>
        ...
        {!course.isEnrolled && (
          ...
          >
            {enrolling ? <Loader2 className="w-5 h-5 animate-spin" /> : <Plus className="w-5 h-5" />}
