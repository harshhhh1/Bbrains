...
import {
  formatCurrency,
  toWeeklySchedule,
} from "@/features/manager/classes/model/classes";

export function ClassPreview({ selectedClass }) {
  return (
    <Card className="border-border/60 shadow-sm">
      <CardHeader>
        <CardTitle>
          {selectedClass ? `${selectedClass.name} timetable` : "Class preview"}
        </CardTitle>
        <CardDescription>
          {selectedClass
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="rounded-xl border border-border/60 p-4">
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  {(selectedClass.subjects || []).map((subject) => (
                    <Badge
                      key={`${selectedClass.id}-subject-${subject}`}
                      variant="secondary"
                    >
                      {subject}
                    </Badge>
              ...
              <div className="rounded-xl border border-border/60 p-4">
                ...
                <p className="mt-3 text-sm text-foreground">
                  {selectedClass.durationValue} {selectedClass.durationUnit} at{" "}
                  {formatCurrency(selectedClass.feePerStudent)} per student
                </p>
              </div>
            </div>
            <WeeklySchedulePanel
