...
import { ResultEntryDrawer } from "./ResultEntryDrawer"
...
export function AssessmentResultsTab({
  ...
  const [drawerStudent, setDrawerStudent] = useState<StudentInfo | null>(null)
  ...
  const loadStudentsAndExams = useCallback(async () => {
    ...
    try {
      ...
      if (examsRes.success && examsRes.data) {
        ...
        const courseExams = (Array.isArray(examsRes.data) ? examsRes.data : []).filter(
          (e: ApiExam) => e.courseId === courseId
        )
  ...
  function openDrawer(student: StudentInfo) {
  ...
  return (
    <div className="space-y-8">
      ...
      {/* No exams warning */}
      {selectedCourseId && !loading && students.length > 0 && exams.length === 0 && (
