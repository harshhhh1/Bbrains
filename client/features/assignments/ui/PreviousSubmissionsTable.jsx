...
export function PreviousSubmissionsTable({ submissions }) {
  const getGradeStyle = (type) => {
    switch (type) {
      case "success":
              ...
              {submissions.map((sub) => (
                ...
                >
                  ...
                  <td className="px-6 py-4">
                    <span
                      className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${getGradeStyle(sub.gradeType)}`}
                    >
                      {sub.grade}
                    </span>
                  </td>
