...
import { marketApi, reviewApi } from "@/services/api/client";
...
export default function MarketProductPageClient() {
  ...
  const allImages = useMemo(() => {
    ...
    if (product.metadata?.images && Array.isArray(product.metadata.images))
      imgs.push(...product.metadata.images);
    if (
      product.metadata?.previewImages &&
      Array.isArray(product.metadata.previewImages)
    )
      imgs.push(...product.metadata.previewImages);
    return [...new Set(imgs)];
  }, [product]);
  ...
  const checkCanReview = async (pid) => {
    try {
  ...
  const handleSubmitReview = async () => {
    ...
    try {
      ...
      const resp = await reviewApi.createReview(product.id, {
        rating: newRating,
        comment: newComment,
      });
      if (resp.success) {
  ...
  const sellerName = sellerDetails?.firstName
    ? `${sellerDetails.firstName} ${sellerDetails.lastName || ""}`
    : product.creator?.username || "Verified Agent";
    ...
    <DashboardContent className="mx-auto w-full max-w-[1400px] p-6 md:p-12 space-y-12">
      <div className="hidden md:flex items-center gap-4 translate-x-[-8px]">
        <Link href="/market">
          <Button
            variant="ghost"
            className="rounded-2xl h-12 px-5 hover:bg-muted text-muted-foreground group"
          >
          ...
          <div className="space-y-6">
            <div className="flex items-center justify-between">
              ...
              <div className="hidden md:flex items-center gap-2">
                <Button
                  variant="ghost"
                  size="icon"
                  className="h-10 w-10 rounded-xl bg-card hover:bg-muted"
                >
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-6">
                ...
                <span className="text-sm font-bold text-muted-foreground uppercase tracking-widest">
                  {product.reviewCount || 0} Verifications
                </span>
              </div>
            </div>
            ...
            <div className="flex items-center gap-3">
              {isOutOfStock ? (
                <Badge
                  variant="destructive"
                  className="bg-red-500/10 text-red-500 border-red-500/20 text-[10px] font-black uppercase tracking-widest px-3 py-1 h-8 rounded-xl"
                >
                  Out of Stock
                </Badge>
              ) : (
            ...
            <div className="p-10 rounded-[2.5rem] bg-card border border-border shadow-inner flex flex-col gap-1 relative overflow-hidden group">
              ...
              <span className="text-[10px] font-black uppercase tracking-widest text-muted-foreground mb-1 ml-1">
                Item Value
              </span>
              <div className="flex items-baseline gap-3">
                <span className="text-7xl font-black text-foreground tracking-tighter tabular-nums leading-none">
                  {product.price}
                </span>
            ...
            {alreadyOwned && product.productType === "digital" && (
              <div className="p-5 rounded-2xl bg-emerald-500/10 border border-emerald-500/20 flex items-center gap-4">
                ...
                <p className="text-xs font-bold text-emerald-500 leading-relaxed">
                  This digital record is already assigned to your account and is
                  accessible in your Inventory.
                </p>
              </div>
            )}
            ...
            <div className="space-y-6 pt-4">
              <div className="flex items-center gap-6">
                <div className="flex items-center bg-card rounded-2xl border border-border p-1.5">
                  ...
                  <span className="w-12 text-center text-xl font-black text-foreground tabular-nums">
                    {quantity}
                  </span>
                  <Button
                    variant="ghost"
                ...
                <div>
                  ...
                  <p className="text-2xl font-black text-foreground tracking-tight">
                    {Number(product.price) * quantity} B-Coins
                  </p>
                </div>
              </div>
              ...
              <div className="p-5 rounded-[2rem] bg-muted/20 border border-border/50 flex flex-col gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-muted-foreground leading-relaxed">
                    Item delivery completes immediately after payment.
                  </p>
                </div>
              </div>
              <div className="p-5 rounded-[2rem] bg-muted/20 border border-border/50 flex flex-col gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-muted-foreground leading-relaxed">
                    Secured via end-to-end secure verification.
                  </p>
                </div>
              </div>
            </div>
      ...
      <div className="max-w-4xl mx-auto space-y-20 border-t border-border pt-20">
        <div className="space-y-6">
          ...
          <p className="text-xl text-muted-foreground leading-relaxed font-medium">
            {product.description ||
              "The provider has not supplied detailed technical specifications for this unit."}
          </p>
        </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-10 items-center p-12 rounded-[3rem] bg-muted/20 border border-border/50">
              <div className="md:col-span-4 text-center md:text-left space-y-3">
                ...
                <p className="text-[10px] font-black text-muted-foreground uppercase tracking-[0.2em]">
                  {reviewStats.totalReviews} Total Verifications
                </p>
              </div>
                ...
                {[5, 4, 3, 2, 1].map((star) => {
                  ...
                  return (
                    <div key={star} className="flex items-center gap-6">
                      ...
                      <span className="w-10 text-xs font-black text-muted-foreground/50">
                        {Math.round(pct)}%
                      </span>
                    </div>
                  );
            ...
            <div className="p-10 rounded-[2.5rem] bg-card border-2 border-primary/20 animate-in slide-in-from-top-4 duration-500 space-y-8 shadow-2xl">
              ...
              <div className="flex gap-4">
                <Button
                  ...
                  disabled={
                    newRating === 0 || !newComment.trim() || submittingReview
                  }
            ...
            {reviews.length === 0 ? (
              <div className="py-24 text-center bg-muted/10 rounded-[3rem] border-2 border-dashed border-border/40">
                ...
                <p className="text-muted-foreground font-black uppercase tracking-[0.2em] text-xs">
                  List Empty: No Verdicts Recorded
                </p>
              </div>
            ) : (
              reviews.map((rev) => (
                ...
                >
                  <div className="flex items-center justify-between mb-8">
                    <div className="flex items-center gap-5">
                      <div className="w-14 h-14 rounded-2xl bg-muted/50 flex items-center justify-center text-xl font-black text-primary border border-border/40 shadow-inner">
                        {rev.user?.userDetails?.firstName?.[0] ||
                          rev.user?.username?.[0] ||
                          "?"}
                      </div>
                      <div>
                        <p className="font-black text-foreground text-lg tracking-tight">
                          {rev.user?.userDetails?.firstName
                            ...
                            : rev.user?.username || "Verified Agent"}
                        </p>
                  ...
                  <p className="text-lg text-muted-foreground leading-relaxed font-medium pl-1">
                    {rev.comment}
                  </p>
                </div>
              ))
