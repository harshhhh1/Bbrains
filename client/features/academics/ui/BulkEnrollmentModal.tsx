...
export function BulkEnrollmentModal({ open, onOpenChange, course, onSuccess }: BulkEnrollmentModalProps) {
    ...
    const filteredStudents = useMemo(() => {
        return students.filter(s => {
            ...
            return fullName.includes(query) || s.username.toLowerCase().includes(query) || s.email.toLowerCase().includes(query);
        });
    }, [students, search]);
    ...
    const handleSelectAll = () => {
        const currentlyVisibleUnenrolled = filteredStudents.filter(s => !alreadyEnrolledIds.has(s.id));
        const allVisibleAlreadySelected = currentlyVisibleUnenrolled.every(s => selectedIds.has(s.id));
    ...
    const handleEnroll = async () => {
        ...
        try {
            setSubmitting(true);
            const res = await academicApi.enrollBulk(course.id, Array.from(selectedIds));
            if (res.success) {
                toast.success(res.message || `Enrolled ${selectedIds.size} students successfully`);
                setSelectedIds(new Set());
                fetchData();
                onSuccess?.();
            } else {
                    ...
                    <DrawerHeader className="border-b border-border/60 p-6 text-left">
                        <div className="flex items-start justify-between gap-4">
                            <div className="space-y-2">
                                ...
                                <DrawerDescription>
                                    Bulk enroll students to <span className="font-semibold text-foreground">{course?.name}</span>
                                </DrawerDescription>
                            </div>
                            <DrawerClose asChild>
                    ...
                    <div className="flex-1 overflow-hidden flex flex-col p-6 space-y-4">
                        ...
                        <div className="flex items-center justify-between">
                            ...
                            <Button variant="link" size="sm" onClick={handleSelectAll} className="h-auto p-0 text-xs">
                                {filteredStudents.filter(s => !alreadyEnrolledIds.has(s.id)).every(s => selectedIds.has(s.id)) ? "Deselect All" : "Select All"}
                            </Button>
                        </div>
                                    ...
                                    return (
                                        ...
                                        >
                                            <Checkbox 
                                                ...
                                                disabled={isEnrolled}
                                                onCheckedChange={() => !isEnrolled && toggleStudent(student.id)}
                                            />
                                            ...
                                            {isEnrolled ? (
                                                ...
                                                <Badge className="text-[10px] h-5 bg-primary/20 text-primary hover:bg-primary/20 border-none">Selected</Badge>
                                            )}
                                        </div>
