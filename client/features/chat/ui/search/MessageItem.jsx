...
import { getAvatarColor, getInitials } from "@/features/chat/model/utils";
import { UserBadge } from "@/features/chat/ui/ChatPrimitives";

export const MessageItem = React.memo(function MessageItem({ msg }) {
  ...
  return (
    <div className="group relative flex gap-4 px-4 py-1.5 hover:bg-muted/50">
      <Avatar className="mt-0.5 h-10 w-10 shrink-0 rounded-full border border-border shadow-sm">
        <AvatarFallback
          className={`text-xs font-bold text-white ${getAvatarColor(msg.user.name)}`}
        >
