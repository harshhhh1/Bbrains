...
export const MessageItem = React.memo(function MessageItem({
  ...
  const isMentionedById = Boolean(
    currentUserId && msg.mentionedUserIds?.includes(currentUserId),
  );
  const isMentionedByUsername = Boolean(
    currentUsername &&
    msg.mentions?.some(
      (mention) => mention.toLowerCase() === currentUsername.toLowerCase(),
    ),
  );
  ...
  const content = useMemo(() => {
    const mentionSet = new Set(
      (msg.mentions || []).map((m) => m.toLowerCase()),
    );
  ...
  const isTeacher =
    msg.user.badge?.toLowerCase() === "teacher" ||
    msg.user.badge?.toLowerCase() === "mod";
  const authorNameStyle = isTeacher
          ...
          {msg.replyTo && typeof msg.replyTo !== "string" && (
            <div className="flex items-center gap-2 mb-0.5 ml-[46px] relative h-6">
              ...
              <Avatar className="w-4 h-4 shrink-0">
                <AvatarImage
                  src={msg.replyTo.avatar || undefined}
