...
import type { SubjectChapterProgress } from "@/services/api/client";

interface ClassProgressHubProps {
  courseLoading: boolean;
  selectedCourse: any;
  chapterProgressDraft: SubjectChapterProgress[];
  teacherSubjects: string[];
  savingChapterProgress: boolean;
  hasChapterDraftChanges: boolean;
  onUpdateProgress: (subject: string, field: "completedChapters" | "totalChapters", value: number) => void;
  onSave: () => Promise<void>;
}
    ...
    <Card className="border-border/60">
      ...
      <CardContent>
        {courseLoading ? (
            ...
            <div className="space-y-3">
              {teacherSubjects.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-10 text-center border border-dashed border-border/60 rounded-2xl bg-muted/20">
                  ...
                  <p className="text-sm font-semibold text-muted-foreground">No subjects assigned yet</p>
                  <p className="text-xs text-muted-foreground/50 mt-1">
                    Please contact the admin to assign subjects to your profile.
                  </p>
                </div>
              ) : (() => {
                const filteredProgress = chapterProgressDraft.filter((entry) =>
                        ...
                        <div className="flex items-center gap-2">
                          <div className="flex items-center bg-background rounded-lg border border-border/50 p-0.5">
                            ...
                            <div className="w-10 text-center">
                              <span className="text-sm font-bold">{entry.completedChapters}</span>
                            </div>
                            <Button
                              variant="ghost"
                              size="icon"
                              ...
                              onClick={() =>
                                onUpdateProgress(
                                  entry.subject,
                                  "completedChapters",
                                  Math.min(entry.totalChapters, entry.completedChapters + 1)
                                )
                              }
                            >
          ...
          <p className="text-center py-4 text-muted-foreground text-sm italic">Select a class</p>
        )}
      </CardContent>
    </Card>
  );
