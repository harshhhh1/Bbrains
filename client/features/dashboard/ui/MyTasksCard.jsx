...
import { canSubmitAssignment } from "@/features/assignments/model";
...
export function MyTasksCard() {
  ...
  const fetchAssignments = async () => {
    try {
      ...
      if (assignmentResponse.success && assignmentResponse.data) {
        const submissions =
          submissionsResponse.success && submissionsResponse.data
        ...
        const enrichedAssignments = assignmentResponse.data
          .map((assignment) => ({
            ...assignment,
  ...
  const handleUploadClick = (assignment, e) => {
    e.stopPropagation();
    ...
    <Card className="h-full border-border/40 shadow-sm overflow-hidden">
      <CardHeader className="flex flex-row items-center justify-between pb-2">
        ...
        {mounted &&
          ...
          canSubmitAssignment(assignments[0]) && (
              ...
              return (
                ...
                >
                  <div className="flex items-center gap-3">
                    <div
                      className={`h-8 w-8 rounded-lg flex items-center justify-center ${submissionStatus ? submissionStatus.bgColor : "bg-brand-purple/10"}`}
                    >
                      {submissionStatus ? (
                        <submissionStatus.icon
                          className={`h-4 w-4 ${submissionStatus.color}`}
                        />
                      ) : (
