...
export function LevelWidget({
  ...
  const [realtimeLevel, setRealtimeLevel] = useState(null);
  const [realtimeXp, setRealtimeXp] = useState(null);
  ...
  const safeLevel =
    realtimeLevel !== null
      ? realtimeLevel
      : Number.isFinite(initialLevel) && initialLevel > 0
        ? initialLevel
        : 1;
  const safeXp =
    realtimeXp !== null
      ? realtimeXp
      : Number.isFinite(initialXp) && initialXp > 0
        ? initialXp
        : 0;
  ...
  const progressStartXp =
    safeXp >= resolvedCurrentLevelXp ? resolvedCurrentLevelXp : 0;
  const xpNeeded = hasNextLevel
    ? Math.max(resolvedNextLevelXp - progressStartXp, 0)
    : 0;
  const xpInLevel = Math.max(safeXp - progressStartXp, 0);
  const xpRemaining = hasNextLevel
    ? Math.max(resolvedNextLevelXp - safeXp, 0)
    : 0;
  const progress =
    hasNextLevel && xpNeeded > 0
