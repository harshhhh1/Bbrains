...
export const LeaderboardCard = memo(function LeaderboardCard({
  ...
  const [myPosition, setMyPosition] = useState(initialMyPosition || null);
  ...
  const fetchLeaderboard = useCallback(async () => {
    ...
    try {
      ...
      if (lbResponse.success && lbResponse.data) {
        setLeaderboard(transformLeaderboard(lbResponse.data).slice(0, 5));
      } else {
        setError(lbResponse.message || "Failed to load leaderboard");
      }

      if (meResponse.success && meResponse.data) {
        const transformed = transformLeaderboard([meResponse.data]);
  ...
  const formatRank = (rank) => `${rank}${getRankSuffix(rank)}`;

  const isUserInTop =
    myPosition && leaderboard.some((user) => user.id === myPosition.id);

  const RankItem = ({ user, isMe = false }) => (
    <div
        ...
        <div className="flex flex-col">
          ...
          <div className="flex items-center gap-2 mt-1">
            <span className="text-[10px] text-foreground/70 flex items-center gap-0.5">
              <TrendingUp className="h-2.5 w-2.5" /> {user.xp.toLocaleString()}{" "}
              XP
            </span>
            <span className="text-[10px] text-foreground/70 flex items-center gap-0.5">
              <img src="/bcoin.svg" className="h-2.5 w-2.5 mb-0.5" alt="" />{" "}
              ...
              <>
                <div className="relative py-2">
                  ...
                  <div className="relative flex justify-center">
                    <span className="bg-card px-2 text-[10px] text-muted-foreground uppercase tracking-widest font-medium">
                      Your Position
                    </span>
                  </div>
                </div>
