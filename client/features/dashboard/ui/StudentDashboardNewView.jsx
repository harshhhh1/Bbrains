...
import { LeaderboardCard } from "@/features/dashboard/ui/LeaderboardCard";
...
function SketchPanel({
  ...
  return (
    <div className={cn("relative pt-5", className)}>
      ...
      <div
        className={cn(
          "relative transition-transform duration-300 hover:rotate-0",
          tilt,
        )}
      >
        <div
...
export function StudentDashboardNewView({
  ...
  const quickStats = [
    ...
    },
    {
      ...
      note:
        streakDays > 0
          ...
          : "Claim today and start the chain",
      color: "default",
      tilt: "rotate-1",
      textClassName: "text-hand-pencil",
      noteClassName: "text-hand-pencil/75",
    ...
    },
    {
      ...
      note: leaderboardRank
        ? "Keep climbing the board"
        : "Jump into the leaderboard this week",
      color: "blue",
      tilt: "-rotate-1",
  ...
  const deskNotes = [
    ...
    },
    {
      title: "Next event",
      body: nextEvent
        ? `${nextEvent.title || "Event"} on ${formatShortDate(nextEvent.date)}`
      ...
      <div className="relative overflow-hidden rounded-[42px] border-[3px] border-hand-pencil bg-hand-paper bg-paper-texture [background-size:18px_18px] shadow-[10px_10px_0px_0px_rgba(45,45,45,0.12)]">
            ...
            <div className="grid gap-4 md:grid-cols-3">
              {quickStats.map((stat) => (
                ...
                >
                  <p
                    className={cn(
                      "font-patrick text-sm uppercase tracking-[0.18em]",
                      stat.labelClassName,
                    )}
                  >
                    {stat.label}
                  ...
                  <p
                    className={cn(
                      "mt-3 font-kalam text-3xl font-bold",
                      stat.textClassName,
                    )}
                  >
                    {stat.value}
        ...
        <section className="relative space-y-8 px-5 py-8 md:px-8">
          ...
          <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-5">
            <SketchPanel tilt="-rotate-1" accentClassName="bg-hand-yellow/20">
              <LevelWidget
                ...
                currentLevelXp={
                  dashboardData?.stats?.nextLevelRequiredXp
                    ? dashboardData?.stats?.nextLevelRequiredXp - 100
                    : undefined
                }
              />
            </SketchPanel>
          ...
          <div className="grid gap-5 lg:grid-cols-2">
            <SketchPanel tilt="-rotate-1" accentClassName="bg-hand-red/10">
              <AnnouncementsCard
                initialAnnouncements={dashboardData?.announcements?.slice(0, 5)}
              />
            </SketchPanel>

            <SketchPanel tilt="rotate-1" accentClassName="bg-hand-yellow/20">
              <UpcomingEventsCard
                initialEvents={dashboardData?.events?.slice(0, 5)}
              />
            </SketchPanel>
          </div>
