...
function PercentageBar({ label, value, total, tone }) {
...
function OverviewLinkCard({ href, className = "", children }) {
  return (
    <Link href={href} className="block min-w-0">
      <Card
        className={`min-w-0 border-border/60 shadow-sm transition-all hover:-translate-y-0.5 hover:border-primary/40 hover:shadow-md ${className}`}
      >
        {children}
      </Card>
    </Link>
...
function TransactionRow({ note, date, amount, currency, type }) {
  return (
...
export function OverviewClient({ stats }) {
  const fullName = getFullName(
    stats.admin.firstName,
    stats.admin.lastName,
    stats.admin.username || "Administrator",
  );
                ...
                <div className="space-y-3">
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <span className="rounded-full bg-muted px-3 py-1 text-[11px] font-semibold uppercase tracking-[0.18em] text-muted-foreground">
                      {stats.people.studentToTeacherRatio
                        ? `${stats.people.studentToTeacherRatio}:1 student-teacher ratio`
                        : "Ratio unavailable"}
                    </span>
                  </div>
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="max-w-3xl text-sm leading-6 text-muted-foreground">
                      All the useful admin information stays on one screen now,
                      grouped into denser cards so staffing, revenue, and
                      institution details are easy to scan without the page
                      feeling empty.
                    </p>
                  </div>
                </div>
            ...
            <CardContent className="grid h-full gap-3 p-4 sm:grid-cols-2 xl:grid-cols-1">
              <div className="rounded-2xl border border-border/60 bg-background/80 p-4">
                ...
                <p className="mt-1 text-sm text-muted-foreground">
                  Keep tabs on operations without opening extra pages.
                </p>
              </div>
              ...
              <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-2">
                <CompactStat
                  ...
                  tone="border-blue-200/60 bg-blue-500/5"
                />
                <CompactStat
                  label="Managers"
                  value={stats.people.managers}
                  tone="border-amber-200/60 bg-amber-500/5"
                />
                <CompactStat
                  label="Staff"
                  value={stats.people.staff}
                  tone="border-emerald-200/60 bg-emerald-500/5"
                />
                <CompactStat
                  label="Students"
              ...
              <CardHeader className="space-y-1 pb-3">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <CardDescription>
                      Gender mix and enrollment balance at a glance.
                    </CardDescription>
                  </div>
              ...
              <CardContent className="space-y-4 pt-0">
                <div className="grid gap-3 sm:grid-cols-3">
                  <CompactStat
                    ...
                    tone="border-blue-200/60 bg-blue-500/10"
                  />
                  <CompactStat
                    label="Girls"
                    value={stats.students.girls}
                    tone="border-pink-200/60 bg-pink-500/10"
                  />
                  <CompactStat
                    label="Other"
                    value={stats.students.others}
                    tone="border-slate-200/70 bg-muted/60"
                  />
                </div>
              ...
              <CardHeader className="space-y-1 pb-3">
                <div className="flex items-start justify-between gap-3">
                  <div>
                    ...
                    <CardDescription>
                      Received, accrued, receivable, and fee base in one card.
                    </CardDescription>
                  </div>
                  <ArrowUpRight className="size-4 text-muted-foreground" />
                </div>
              ...
              <CardContent className="space-y-4 pt-0">
                <div className="rounded-[1.25rem] bg-emerald-500/10 p-4">
                  ...
                  <p className="mt-3 text-3xl font-bold tracking-tight text-foreground">
                    {formatCurrency(
                      stats.finance.receivedIncome,
                      stats.finance.currency,
                    )}
                  </p>
                  <p className="mt-1 text-sm text-muted-foreground">
                    {stats.finance.receivedSource === "config"
                      ? "Using configured totals"
                      : "Using fee-tagged successful transactions"}
                  </p>
                </div>
                ...
                <div className="grid gap-3 sm:grid-cols-2">
                  ...
                  <CompactStat
                    label="Receivable"
                    value={formatCurrency(
                      stats.finance.receivableIncome,
                      stats.finance.currency,
                    )}
                    tone="border-border/60 bg-background/75"
                  />
          ...
          <OverviewLinkCard href="/admin/finance">
            <CardHeader className="space-y-1 pb-3">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <CardDescription>
                    Recent entries stay visible here so finance movement is
                    always close by.
                  </CardDescription>
                </div>
                <ArrowUpRight className="size-4 text-muted-foreground" />
            ...
            <CardContent className="space-y-3 pt-0">
              {stats.finance.latestTransactions.length > 0 ? (
                stats.finance.latestTransactions
                  .slice(0, 4)
          ...
          <OverviewLinkCard href="/settings">
            <CardHeader className="space-y-1 pb-3">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <CardDescription>
                    Identity, contact details, wallet context, and account
                    controls for current admin.
                  </CardDescription>
                </div>
                <ArrowUpRight className="size-4 text-muted-foreground" />
              </div>
            ...
            <CardContent className="space-y-4 pt-0">
              ...
              <div className="rounded-[1.25rem] border border-border/60 px-4 py-1">
                ...
                <InfoRow
                  label="Phone"
                  value={stats.admin.phone || "Not provided"}
                />
                <InfoRow
                  label="Wallet"
                  value={formatCurrency(
                    stats.admin.walletBalance,
                    stats.finance.currency,
                  )}
                />
                <InfoRow
                  label="Joined"
                  value={formatDate(stats.admin.createdAt)}
                />
                <InfoRow
                  label="Bio"
                  value={stats.admin.bio || "No bio added yet"}
                />
              </div>
            </CardContent>
          </OverviewLinkCard>
            ...
            <CardHeader className="space-y-1 pb-3">
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <CardDescription>
                    Linked organization details without needing a separate page
                    open.
                  </CardDescription>
                </div>
                <ArrowUpRight className="size-4 text-muted-foreground" />
              ...
              {stats.institution ? (
                <>
                  <div className="rounded-[1.25rem] bg-primary/10 p-4">
                    <div className="flex items-start gap-3">
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <p className="text-sm text-muted-foreground">
                          Registration No. {stats.institution.regNo}
                        </p>
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="grid gap-3 sm:grid-cols-2">
                    ...
                    <div className="rounded-2xl border border-border/60 bg-background/75 p-4">
                      ...
                      <p className="mt-2 text-sm font-medium text-foreground">
                        {formatDate(stats.institution.createdAt)}
                      </p>
                    </div>
                  </div>
                  ...
                  <div className="rounded-[1.25rem] border border-border/60 px-4 py-1">
                    <InfoRow
                      label="Institution"
                      value={stats.institution.name}
                    />
                    <InfoRow
                      label="Registration"
                      value={stats.institution.regNo}
                    />
                    <InfoRow
                      label="Address"
                      value={
                        stats.institution.address || "No address on record"
                      }
                    />
                  </div>
                </>
                ...
                <div className="rounded-[1.25rem] border border-dashed border-border/70 px-4 py-5 text-sm text-muted-foreground">
                  No university or college profile is linked to this admin
                  account yet.
                </div>
              )}
            </CardContent>
