...
import { dashboardApi, streakApi } from "@/services/api/client";
import { useRouter } from "next/navigation";
import { TOTAL_DAYS, XP_REWARDS } from "@/features/dashboard/config/rewards";
import { useUser } from "@/context/user-context";

export function DailyRewardCard({ initialStreak }) {
  const router = useRouter();
  const { refreshUser } = useUser();
  const [streak, setStreak] = useState(initialStreak || null);
  ...
  const syncClaimState = (streakData) => {
    if (!streakData) {
        ...
        {loading ? (
          ...
          <>
            ...
            <div className="flex gap-1 mt-4">
              {Array.from({ length: TOTAL_DAYS }, (_, index) => index + 1).map(
                (d) => (
                  <div
                    key={d}
