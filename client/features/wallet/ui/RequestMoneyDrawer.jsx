...
export function RequestMoneyDrawer({ open, onOpenChange, onSuccess }) {
  ...
  const handleSendRequest = async () => {
    if (!requestTo) {
      setErrors({ recipient: "Select a source" });
      return;
    }
    ...
    if (!reason.trim()) {
      toast.error("Provision a reason for request");
      return;
    }
        ...
        <div className="flex h-[100dvh] max-h-[100dvh] flex-col overflow-hidden">
            ...
            <div className="flex items-start justify-between gap-4">
              <div className="space-y-2">
                ...
                <DrawerDescription className="font-medium">
                  Request B-Coins from another user.
                </DrawerDescription>
              </div>
              <DrawerClose asChild>
                <Button
          ...
          <DrawerFooter className="border-t border-border/60 p-8 bg-muted/5">
            <Button
              ...
              disabled={!requestTo || !amount || !reason}
            >
              Send Request
            </Button>
          </DrawerFooter>
