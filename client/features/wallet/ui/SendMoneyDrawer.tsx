...
import type { SearchUser } from "@/features/wallet/api/data";
import type { ValidationErrors } from "@/lib/validation";

interface SendMoneyDrawerProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onNext: (data: { recipientId: string; amount: number; note: string }) => void;
          ...
          <DrawerHeader className="border-b border-border/60 p-6 text-left">
            <div className="flex items-start justify-between gap-4">
              <div className="space-y-2">
                <DrawerTitle className="text-2xl font-black tracking-tight">Dispatch Assets</DrawerTitle>
                <DrawerDescription className="font-medium">Transfer B-Coins to another verified agent.</DrawerDescription>
              </div>
              <DrawerClose asChild>
            ...
            {showInlineScan ? (
              <div className="space-y-6">
                <div className="flex items-center justify-between">
                  <p className="text-xs font-black uppercase tracking-widest text-primary">Optical Scan Active</p>
                  <Button variant="ghost" size="sm" onClick={() => setShowInlineScan(false)} className="rounded-xl h-8 font-bold">Cancel</Button>
                </div>
              ...
              <>
                <div className="space-y-3">
                  <div className="flex items-center justify-between ml-1">
                    <Label className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground">Target Recipient</Label>
                    <Button variant="ghost" size="sm" className="h-6 gap-1 text-[10px] font-black uppercase tracking-widest text-primary hover:bg-primary/5" onClick={() => setShowInlineScan(true)}>
                  ...
                  {selectedUser ? (
                    <div className="flex items-center gap-4 bg-primary/5 rounded-2xl p-4 border border-primary/20 shadow-sm animate-in zoom-in-95 duration-300">
                      ...
                      <div className="flex-1 min-w-0">
                         ...
                         <p className="text-[10px] font-black uppercase tracking-widest text-primary/60">Verified Identity</p>
                      </div>
                      ...
                      {showDropdown && (searchResults.length > 0 || searching) && (
                        <div className="absolute z-50 w-full mt-2 bg-card border border-border/60 rounded-2xl shadow-2xl p-2 max-h-60 overflow-y-auto animate-in fade-in slide-in-from-top-2 duration-300">
                           {searching ? <div className="p-4 text-center text-xs font-bold text-muted-foreground uppercase tracking-widest animate-pulse">Syncing...</div> :
                             searchResults.map(u => (
                               <button key={u.id} className="flex items-center gap-3 w-full p-3 hover:bg-muted/50 rounded-xl transition-colors text-left" onClick={() => { setSelectedUser(u); setSendTo(u.id); setShowDropdown(false); }}>
                  ...
                  {errors.recipient && <p className="text-[10px] font-bold text-rose-500 ml-1 uppercase tracking-wider">{errors.recipient}</p>}
                </div>

                <div className="space-y-3">
                   <Label className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground ml-1">Asset Volume</Label>
                   <div className="relative">
                      ...
                      <Input 
                        ...
                        value={sendAmount} 
                        onChange={(e) => { setSendAmount(e.target.value); setErrors(p => ({ ...p, amount: undefined })); }}
                      />
                   </div>
                   {errors.amount && <p className="text-[10px] font-bold text-rose-500 ml-1 uppercase tracking-wider">{errors.amount}</p>}
                </div>
