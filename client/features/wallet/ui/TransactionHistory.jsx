...
export function TransactionHistory({ transactions, loading, error }) {
  const [txnSearch, setTxnSearch] = useState("");
  const [showTxnReceipt, setShowTxnReceipt] = useState(null);
  ...
  return (
    <>
                ...
                >
                  <CardContent className="p-4 flex items-center justify-between">
                    <div className="flex items-center gap-4 min-w-0">
                      <div className="relative">
                        <Avatar className="w-12 h-12 border-2 border-background shadow-sm ring-1 ring-border group-hover:ring-primary/30 transition-all">
                          ...
                          <AvatarFallback
                            name={username}
                            className="bg-gradient-to-br from-primary/5 to-primary/10 text-primary text-[10px] font-black tracking-tighter"
                          >
                            {username.substring(0, 2).toUpperCase()}
                          </AvatarFallback>
                        </Avatar>
                        <div
                          className={`absolute -bottom-1 -right-1 w-5 h-5 rounded-full border-2 border-background flex items-center justify-center shadow-sm ${credit ? "bg-green-500" : "bg-destructive"}`}
                        >
                          {credit ? (
                    ...
                    <div className="text-right shrink-0">
                      <p
                        className={`font-black text-xl tracking-tighter ${credit ? "text-green-600 dark:text-green-400" : "text-destructive"}`}
                      >
                        {credit ? "+" : "-"}
                        {txn.amount ?? 0}
                      </p>
                      <p className="text-[10px] font-bold text-muted-foreground/50 uppercase tracking-widest mt-1 flex items-center justify-end gap-1">
                        <img
                          src="/bcoin.svg"
                          className="h-3 w-3 opacity-50 grayscale"
                          alt=""
                        />
                        BCoins
                      </p>
      ...
      <Dialog
        open={!!showTxnReceipt}
        onOpenChange={() => setShowTxnReceipt(null)}
      >
        <DialogContent className="sm:max-w-sm">
          ...
          <div className="text-center space-y-4 pt-4">
            <div
              className={`w-16 h-16 rounded-full flex items-center justify-center mx-auto ${isCredit(showTxnReceipt?.type) ? "bg-green-500/10" : "bg-destructive/10"}`}
            >
            ...
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                {showTxnReceipt?.transactionDate
                  ? formatTxnDate(showTxnReceipt.transactionDate)
                  : ""}
              </p>
            </div>
            <div className="bg-muted/50 rounded-xl p-4 space-y-3 text-sm text-left border border-muted">
              <div className="flex justify-between items-center pb-2 border-b border-muted">
                ...
                <span
                  className={`font-bold text-lg ${isCredit(showTxnReceipt?.type) ? "text-green-600" : "text-destructive"}`}
                >
                  {isCredit(showTxnReceipt?.type) ? "+" : "-"}
                  {showTxnReceipt?.amount ?? 0} B-Coins
                </span>
              </div>
                ...
                <div className="pt-2 border-t border-muted">
                  ...
                  <div className="flex items-center gap-2">
                    <Avatar className="w-6 h-6">
                      <AvatarFallback
                        name={
                          showTxnReceipt.relatedUser?.username ||
                          showTxnReceipt.user?.username
                        }
                        className="text-[10px]"
                      >
                        {(
