...
export function SpendingsChart({ transactions }) {
  ...
  const chartData = useMemo(() => {
    ...
    transactions.forEach((txn) => {
      const date = new Date(txn.transactionDate || txn.createdAt || new Date());
      const amount = Number(txn.amount || 0);
      const isCredit = ["credit", "received", "deposit"].includes(
      ...
      <CardHeader className="flex flex-row items-center justify-between pb-2">
        ...
        <Tabs value={chartFilter} onValueChange={setChartFilter}>
          <TabsList className="h-8 bg-muted/50 rounded-xl p-1">
            <TabsTrigger
              value="this-week"
              className="text-[10px] uppercase tracking-widest font-black px-3 rounded-lg"
            >
              Week
            </TabsTrigger>
            <TabsTrigger
                      ...
                      <div className="rounded-2xl border border-border/60 bg-card/95 p-4 shadow-2xl backdrop-blur-md animate-in fade-in zoom-in-95 duration-200">
                        ...
                        <div className="space-y-2">
                          {payload.map((entry) => (
                            <div
                              key={entry.name}
                              className="flex items-center gap-4 justify-between min-w-[140px]"
                            >
