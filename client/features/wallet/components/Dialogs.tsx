...
import { walletApi, WalletData, MoneyRequest } from "@/services/api/client";
import { SendMoneyDrawer } from "./SendMoneyDrawer";
import { ReceiveMoneyDrawer } from "./ReceiveMoneyDrawer";
import { ScanPayDrawer } from "./ScanPayDrawer";
import { RequestMoneyDrawer } from "./RequestMoneyDrawer";
import { PendingRequestsDrawer } from "./PendingRequestsDrawer";
import { TransactionReceiptDrawer } from "./TransactionReceiptDrawer";
...
export function WalletDialogs({
  ...
  const [transferData, setTransferData] = useState<{ recipientId: string; amount: number; note: string } | null>(null);
  ...
  const handleSendNext = (data: { recipientId: string; amount: number; note: string }) => {
  ...
  const handleAcceptRequest = (request: MoneyRequest) => {
  ...
  const handleRequestPinSubmit = async () => {
    ...
    try {
      setIsProcessing(true);
      const res = await walletApi.respondToRequest(pendingAcceptRequest.id, true, requestPin);
      if (res.success) {
        toast.success("Solicitation Authorized");
        setShowRequestPinDialog(false);
        setPendingRequests(p => p.filter(r => r.id !== pendingAcceptRequest.id));
        onTransferSuccess();
      } else {
  ...
  return (
    <>
      ...
      <Drawer open={showPinDialog} onOpenChange={setShowPinDialog}>
        <DrawerContent className="p-0 data-[vaul-drawer-direction=bottom]:max-h-[60vh] before:inset-0 before:rounded-none before:border-border before:bg-background sm:p-0 sm:before:rounded-t-[2.5rem]">
          <DrawerHeader className="border-b border-border/60 p-8 text-center items-center">
            <DrawerTitle className="text-2xl font-black">Verify Authorization</DrawerTitle>
            <DrawerDescription className="font-medium">Enter your secure PIN to authorize transfer of {transferData?.amount} B-Coins.</DrawerDescription>
          </DrawerHeader>
          ...
          <DrawerFooter className="border-t border-border/60 p-8">
             <Button size="lg" className="w-full h-14 rounded-2xl bg-primary hover:bg-primary/90 text-primary-foreground font-black uppercase tracking-widest text-xs shadow-lg shadow-primary/20" onClick={handlePinSubmit} disabled={pin.length < 4 || isProcessing}>
      ...
      <Drawer open={showRequestPinDialog} onOpenChange={setShowRequestPinDialog}>
        <DrawerContent className="p-0 data-[vaul-drawer-direction=bottom]:max-h-[60vh] before:inset-0 before:rounded-none before:border-border before:bg-background sm:p-0 sm:before:rounded-t-[2.5rem]">
          <DrawerHeader className="border-b border-border/60 p-8 text-center items-center">
            <DrawerTitle className="text-2xl font-black">Authorize Solicitation</DrawerTitle>
            <DrawerDescription className="font-medium">Verify credentials to fulfill request for {pendingAcceptRequest?.amount} B-Coins.</DrawerDescription>
          </DrawerHeader>
          <div className="p-10 flex flex-col items-center">
             <Input 
              type="password" 
          ...
          <DrawerFooter className="border-t border-border/60 p-8">
             <Button size="lg" className="w-full h-14 rounded-2xl bg-primary hover:bg-primary/90 text-primary-foreground font-black uppercase tracking-widest text-xs shadow-lg shadow-primary/20" onClick={handleRequestPinSubmit} disabled={requestPin.length < 4 || isProcessing}>
                {isProcessing ? <Loader2 className="w-5 h-5 animate-spin" /> : "Verify & Fulfill"}
             </Button>
          </DrawerFooter>
        </DrawerContent>
      </Drawer>
      ...
      <TransactionReceiptDrawer 
        ...
        data={{ recipientId: transferData?.recipientId || '', amount: transferData?.amount || 0, note: transferData?.note }} 
      />
    </>
  );
}
