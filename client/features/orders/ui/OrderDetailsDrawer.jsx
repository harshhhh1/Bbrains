...
import { Package, QrCode, BookOpen, X } from "lucide-react";
        ...
        <div className="flex h-[100dvh] max-h-[100dvh] flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-8">
            <div className="space-y-4">
              <h4 className="text-[10px] font-black uppercase tracking-[0.2em] text-primary ml-1">
                Item List
              </h4>
              <div className="space-y-3">
                {order.items.map((item) => (
                  <div
                    key={item.id}
                    className="flex items-center gap-4 p-4 rounded-2xl bg-card border border-border/60 shadow-sm group"
                  >
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="font-bold text-foreground text-sm truncate">
                        {item.product?.name || "Academic Unit"}
                      </p>
            ...
            <div className="p-6 rounded-3xl bg-muted/30 border border-border/50 space-y-4">
              ...
              {statusConfig[order.status] && (
                <div className="flex items-center justify-between pt-4 border-t border-border/50">
                  ...
                  <Badge
                    className={cn(
                      "text-[10px] font-black uppercase tracking-widest px-3 py-1 border",
          ...
          <DrawerFooter className="border-t border-border/60 p-6 gap-3">
            {order.orderType !== "digital" &&
              ...
              order.qrCode && (
                <Button
                  onClick={onShowQR}
                  className="h-14 rounded-2xl bg-primary hover:bg-primary/90 text-primary-foreground font-black uppercase tracking-widest shadow-lg shadow-primary/20"
                >
