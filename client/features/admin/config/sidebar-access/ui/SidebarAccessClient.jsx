...
import {
  fetchCollegeSidebarAccess,
  updateCollegeSidebarAccess,
} from "@/services/api/sidebar";
...
const ROLES = ["student", "teacher", "admin", "staff", "manager", "superadmin"];
...
const TOGGLABLE_ITEMS = (() => {
  ...
  return sidebarItems.filter((item) => {
    ...
    if (ADMIN_ONLY_PATHS.some((path) => url === path || url.startsWith(path)))
      return false;
...
export default function SidebarAccessClient() {
  ...
  const handleToggle = (itemKey, role) => {
    setAccessMap((prev) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (!res.success) {
        toast.error(
          res.message || res.error || "Failed to update sidebar access.",
        );
        return;
      }
      toast.success(
        "Sidebar access updated successfully. Refresh to see changes.",
      );
    } catch (error) {
    ...
    <div className="space-y-6 p-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-muted-foreground">
            Configure which roles can see specific pages in the sidebar for your
            college.
          </p>
        </div>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Select a role to manage their sidebar visibility. Default BBrains
            restrictions still apply for system pages.
          </CardDescription>
        </CardHeader>
        <CardContent>
