...
import { ConfigTable } from "@/features/admin/config/ui/ConfigTable";
import { ConfigForm } from "@/features/admin/config/ui/ConfigForm";
import { fetchConfigs } from "@/features/admin/config/api/data";
import {
  getInitFormData,
  getInitFormDataFromConfig,
} from "@/features/admin/config/types";
import { useHasPermission } from "@/components/providers/permissions-provider";

export function ConfigClient({ initialConfigs }) {
  ...
  if (!canManageInstitution) {
    return (
      <div className="flex h-[calc(100vh-4rem)] flex-col items-center justify-center gap-3 text-muted-foreground">
        ...
        <p className="text-xs">
          You need the "Manage Institution" permission to view this page.
        </p>
      </div>
    );
  }
