...
export function StudentForm({ form, onChange, disabled, courses, isEditing }) {
  const filteredCourses = form.collegeId
    ? courses.filter(
        (course) => String(course.college?.id) === String(form.collegeId),
      )
    : [];
  ...
  const classOptions = [
    {
      value: "",
      label: form.collegeId ? "Select class" : "Enter College ID first",
    },
    ...
    <div className="grid grid-cols-2 gap-3">
      ...
      {!isEditing && (
        <p className="col-span-2 text-xs text-muted-foreground">
          This creates a live student login immediately. Share the temporary
          password with the student or ask them to reset it after their first
          sign-in.
        </p>
      )}
    </div>
