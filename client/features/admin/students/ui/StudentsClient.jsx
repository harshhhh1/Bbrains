...
import { StudentsTable } from "@/features/admin/students/ui/StudentsTable";
import { StudentForm } from "@/features/admin/students/ui/StudentForm";
...
export function StudentsClient({ initialStudents }) {
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const res = editing
        ? await api.put(`/user/students/${editing.id}`, payload)
        : await api.post("/user/students", payload);
      ...
      if (res.success && res.data) {
        ...
        setStudents((prev) =>
          editing
            ? prev.map((student) =>
                student.id === editing.id ? res.data : student,
              )
            : [res.data, ...prev],
        );
        setModalOpen(false);
        ...
        toast.error(
          res.message ||
            (editing ? "Failed to update student" : "Failed to create student"),
        );
      }
    } catch (error) {
      console.error(error);
  ...
  return (
    <div className="space-y-4">
      <SectionHeader
        ...
        action={
          canCreateStudent || canManageStudent
            ? {
                label: "Add Student",
