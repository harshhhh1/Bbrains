        ...
        <div className="col-span-2 space-y-4">
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-2 p-3 rounded-2xl border border-dashed border-border/60 bg-muted/30">
              {Array.from(new Set(courses.flatMap((c) => c.subjects || [])))
                .sort()
                .map((subject) => {
                  const isSelected = form.teacherSubjectsText
                    .split(/\r?\n|,/)
                    .map((s) => s.trim())
                    .includes(subject);
                  return (
                    <button
                      ...
                      type="button"
                      onClick={() => {
                        const current = form.teacherSubjectsText
                          .split(/\r?\n|,/)
                          .map((s) => s.trim())
                          .filter(Boolean);
                        let next;
              ...
              {courses.length === 0 && (
                <p className="text-[10px] text-muted-foreground italic">
                  No courses found to extract subjects from.
                </p>
              )}
            </div>
          </div>
          ...
          <FormTextarea
            ...
            onChange={(value) =>
              onChange({ ...form, teacherSubjectsText: value })
            }
    ...
    <>
      ...
      {!isEditing && (
        <p className="text-xs text-muted-foreground mt-4">
          This creates a live login account immediately. Share the temporary
          password with the teacher or have them reset it after first sign-in.
        </p>
      )}
    </>
  );
