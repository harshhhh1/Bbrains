...
export default function RolesPage() {
  ...
  const [userLowestPosition, setUserLowestPosition] = useState(Infinity);
  const [isUserSuperAdmin, setIsUserSuperAdmin] = useState(false);

  const loadData = useCallback(async (collegeId, currentUser) => {
    setLoading(true);
    setLoadError(null);
    try {
      ...
      const formattedRoles = (rolesResp.data ?? []).map((r) => ({
        ...r,
      ...
      const permsResp = await api.get("/roles/permissions");
      if (permsResp.success) {
        setAllPermissions(permsResp.data || []);
      }

      // Fetch users with roles
      const usersResp = await api.get("/roles/users");
      if (usersResp.success && usersResp.data) {
        const formattedUsers = usersResp.data.map((u) => ({
          ...
          roles: u.roles?.map((ur) => ur.role?.id?.toString()) || [],
        }));
      ...
      const isSuper =
        ...
        currentUser.roles?.some(
          (r) => r.role?.name?.toLowerCase() === "superadmin",
        );
      // Get min position from user's roles
      const userRoles = currentUser.roles || [];
      const positions = userRoles.map((ur) => ur.role?.position ?? 1000);
      ...
      setLoadError(
        `Failed to load roles: ${error?.message || "Internal Server Error"}. Check backend logs.`,
      );
    } finally {
      setLoading(false);
    }
    ...
    return (
      <div className="flex h-[calc(100vh-4.5rem)] items-center justify-center py-8 text-foreground">
        <div className="text-center">
          ...
          <p className="text-sm text-muted-foreground">
            This page needs `user.collegeId` to load roles.
          </p>
        </div>
      </div>
    );
