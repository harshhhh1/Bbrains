...
export default function PermissionsTab({
    ...
    allPermissions.forEach((p) => {
      const isEnabled =
        role.permissions?.some(
          (rp) => rp.permission?.key === p.key && rp.enabled,
        ) || false;
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const updates = Object.entries(pendingPermissions).map(
        ([id, enabled]) => ({
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-8 pb-24">
        {isSelectedRoleSuperAdmin && (
          <div className="rounded-xl bg-destructive/10 p-4 text-sm text-destructive border border-destructive/20">
            SuperAdmin has all permissions by default. You cannot modify
        ...
        {Object.entries(grouped).map(([category, perms]) => (
          <div key={category} className="space-y-4">
            ...
            <div className="rounded-xl bg-card border border-border/60 divide-y divide-border/60 shadow-sm overflow-hidden">
              {perms.map((perm) => {
                const isEnabled = isSelectedRoleSuperAdmin
                  ? true
                  : pendingPermissions[perm.id] || false;
