...
export default function ManageMembersTab({
  ...
  const handleSave = async () => {
    ...
    setIsSaving(true);
    try {
      const res = await api.put(`/roles/${role.id}/members`, {
        userIds: pendingMemberIds,
      });
    ...
    <div className="flex h-full flex-col relative">
      ...
      <div className="flex-1 overflow-y-auto p-6 space-y-8 pb-32">
        ...
        {isHierarchyLocked && !isSuperAdmin && (
          <div className="rounded-xl bg-amber-500/10 p-4 text-sm text-amber-600 border border-amber-500/20">
            This role is at or above your own hierarchy position. You cannot
            manage its members.
          </div>
        )}
          ...
          <div className="space-y-1">
            {filteredMembers.length > 0 ? (
              filteredMembers.map((user) => (
                <div
                  key={user.id}
                  className="flex items-center justify-between rounded-xl p-2 hover:bg-muted/50 transition-colors group"
                >
            ...
            <div className="space-y-1 max-h-60 overflow-y-auto pr-2 custom-scrollbar">
              {filteredNonMembers.length > 0 ? (
                filteredNonMembers.slice(0, 20).map((user) => (
                  <div
                    key={user.id}
                    className="flex items-center justify-between rounded-xl p-2 hover:bg-muted/50 transition-colors"
                  >
                    <div className="flex items-center gap-3">
                      <Avatar className="size-9 border border-border/60">
                        <AvatarImage src={user.avatar} alt={user.username} />
      ...
      {hasChanges && (
        <div className="absolute bottom-6 left-1/2 -translate-x-1/2 w-[90%] max-w-2xl animate-in slide-in-from-bottom-5 rounded-2xl bg-card p-4 shadow-2xl border border-border/60 flex items-center justify-between z-50">
          <p className="text-sm text-foreground font-semibold pl-2">
