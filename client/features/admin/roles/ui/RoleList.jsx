...
function SortableRoleItem({ role, isSelected, isLocked, onSelect }) {
  const {
    attributes,
    listeners,
    setNodeRef,
...
export default function RoleList({
  ...
  const handleCreateRole = async () => {
    ...
    try {
      const maxPos = Math.max(
        ...roles.map((r) => (r.position < 100 ? r.position : 0)),
        4,
      );
  ...
  const handleDragEnd = async (event) => {
    ...
    if (draggedRole.name.toLowerCase() === "superadmin" && !isUserSuperAdmin)
      return;
    ...
    try {
      ...
      await Promise.all(
        updates.map((u) => api.put(`/roles/${u.id}`, { position: u.position })),
      );
