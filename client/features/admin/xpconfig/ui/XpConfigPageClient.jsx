...
export default function XpConfigPage() {
  ...
  if (!canManageInstitution) {
    return (
      <div className="flex h-[calc(100vh-4rem)] flex-col items-center justify-center gap-3 text-muted-foreground">
        ...
        <p className="text-xs">
          You need the &quot;Manage Institution&quot; permission to view this
          page.
        </p>
      </div>
    );
  ...
  const handleOpenModal = (level) => {
    if (level) {
      ...
      setFormData({
        ...
        requiredXp: level.requiredXp.toString(),
      });
    } else {
  ...
  return (
    <div className="space-y-6">
      ...
      <DataTable
        columns={columns}
        data={levels.map((l) => ({ ...l, id: l.levelNumber }))}
        loading={isLoading}
        onDelete={(row) => {
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) =>
                setFormData({ ...formData, requiredXp: e.target.value })
              }
              required
            />
          </div>
