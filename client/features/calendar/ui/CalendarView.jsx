...
export function CalendarView({ view, events }) {
  const getTypeStyles = (type) => {
    switch (type) {
      case "class":
        return "bg-blue-50 dark:bg-blue-900/20 border-blue-100 dark:border-blue-800 text-blue-700 dark:text-blue-400";
      case "exam":
        return "bg-red-50 dark:bg-red-900/20 border-red-100 dark:border-red-800 text-red-700 dark:text-red-400";
      case "study":
        return "bg-orange-50 dark:bg-orange-900/20 border-orange-100 dark:border-orange-800 text-orange-700 dark:text-orange-400";
      case "assignment":
        return "bg-purple-50 dark:bg-purple-900/20 border-purple-100 dark:border-purple-800 text-purple-700 dark:text-purple-400";
    }
  };
      ...
      <div className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-2xl overflow-hidden shadow-sm">
        <div className="grid grid-cols-7 bg-gray-50 dark:bg-gray-800/50 border-b border-gray-100 dark:border-gray-800">
          {weekdays.map((d) => (
            <div
              key={d}
              className="p-3 text-center text-xs font-semibold text-gray-500 dark:text-gray-400 uppercase tracking-tighter"
            >
              {d}
            </div>
          ))}
        ...
        <div className="grid grid-cols-7 bg-gray-100 dark:bg-gray-800 gap-px">
          {Array.from({ length: 2 }).map((_, i) => (
            <div
              key={`empty-${i}`}
              className="bg-gray-50/50 dark:bg-gray-800/20 h-32"
            ></div>
          ))}
          {monthDays.map((day) => {
            ...
            return (
              <div
                key={day}
                className="bg-white dark:bg-gray-900 h-32 p-2 relative group hover:bg-gray-50 dark:hover:bg-gray-800/50 transition-colors"
              >
                <span
                  className={`text-xs font-medium ${day === 13 ? "bg-black dark:bg-white text-white dark:text-black w-6 h-6 flex items-center justify-center rounded-full" : "text-gray-500"}`}
                >
                  {day}
                </span>
                <div className="mt-1 space-y-1">
                  {dayEvents.slice(0, 3).map((e) => (
                    <div
                      key={e.id}
                      className={`text-[9px] px-1.5 py-0.5 rounded border truncate ${getTypeStyles(e.type)}`}
                    >
                      {e.title}
                    </div>
                  ))}
      ...
      <div className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-2xl flex flex-col shadow-sm relative overflow-hidden min-h-[600px]">
        ...
        <div className="flex-1 overflow-y-auto custom-scrollbar">
          {HOURS.map((hour) => (
            <div
              key={hour}
              className="border-b border-gray-100 dark:border-gray-800 h-32 flex group"
            >
                ...
                {dayEvents
                  ...
                  .map((e) => (
                    <div
                      ...
                      style={{
                        height: `calc(${e.durationHours * 100}% + 1rem)`,
                      }}
                    >
                      <div className="flex justify-between items-start">
                        ...
                        <span className="text-xs opacity-70 font-medium">
                          ...
                          {formatHour(e.startHour + e.durationHours)}
                        </span>
                      </div>
    ...
    <div className="bg-white dark:bg-gray-900 border border-gray-100 dark:border-gray-800 rounded-2xl overflow-hidden flex flex-col shadow-sm">
      ...
      <div className="grid grid-cols-[60px_repeat(5,1fr)] sm:grid-cols-[80px_repeat(5,1fr)] border-b border-gray-100 dark:border-gray-800 bg-gray-50 dark:bg-gray-800/50">
        ...
        {DAYS.map((day) => (
          <div
            key={day.id}
            className="p-2 sm:p-4 text-center border-l border-gray-100 dark:border-gray-800"
          >
            <span className="block text-xs sm:text-sm font-semibold text-gray-900 dark:text-white">
              {day.name}
            </span>
          ...
          <React.Fragment key={hour}>
            ...
            {DAYS.map((day) => {
              ...
              const slotEvents = events.filter(
                (e) => e.dayOfWeek === day.id && e.startHour === hour,
              );

              return (
                <div
                  key={`${day.id}-${hour}`}
                  className="bg-white dark:bg-gray-900 p-1 sm:p-2 relative h-24 sm:h-28"
                >
                  {slotEvents.map((e) => (
                    <div
                      key={e.id}
                      ...
                      style={{
                        height: `calc(${e.durationHours * 100}% + ${(e.durationHours - 1) * 2}px)`,
                      }}
                    ...
                    >
                      ...
                      {e.durationHours > 0.5 && (
                        <span className="hidden sm:block text-[10px] opacity-80 mt-1 truncate">
                          {e.subtitle}
                        </span>
                      )}
                      {e.durationHours > 0.5 && (
