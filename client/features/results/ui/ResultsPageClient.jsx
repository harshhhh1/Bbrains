...
import { assessmentApi, dashboardApi } from "@/services/api/client";
...
export default function ResultsPage() {
  ...
  const filteredResults = useMemo(() => {
    return results.filter((result) => {
      if (
        ...
        result.assessment.assessmentType !== assessmentTypeFilter
      )
        return false;
      if (
        subjectFilter !== "all" &&
        result.assessment.subject !== subjectFilter
      )
        return false;
  ...
  const resultSubjects = useMemo(() => {
    return Array.from(
      new Set(results.map((result) => result.assessment.subject)),
  ...
  const averageScore = useMemo(() => {
    ...
    const total = results.reduce(
      (sum, result) => sum + resultPercentage(result),
      0,
    );
    ...
    return (
      <DashboardContent>
        <div className="py-40 flex flex-col items-center justify-center gap-3">
          ...
          <p className="text-sm font-black uppercase tracking-widest text-muted-foreground font-mono">
            Syncing Results...
          </p>
        </div>
      </DashboardContent>
      ...
      <header className="flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
        <div className="space-y-2">
          ...
          <p className="text-muted-foreground text-lg font-medium">
            Report card of test and examination results.
          </p>
        </div>
        ...
        <div className="flex flex-col gap-6 lg:flex-row lg:items-center lg:justify-between">
          <div className="space-y-1">
            ...
            <p className="text-sm font-medium text-muted-foreground">
              Detailed breakdown of your exam scores.
            </p>
          </div>

          <div className="flex flex-col sm:flex-row gap-3">
            <div className="flex flex-col gap-1.5">
              <span className="text-[10px] font-black uppercase text-muted-foreground ml-1 tracking-widest">
                Type
              </span>
              <Select
          ...
          <Card className="border-2 border-dashed border-border/40 bg-muted/10 rounded-[2.5rem] py-24">
            <div className="flex flex-col items-center justify-center text-center px-6">
              ...
              <p className="text-muted-foreground mt-2 max-w-xs font-medium">
                Adjust your criteria or check back after the next results.
              </p>
            </div>
          </Card>
        ) : (
