...
export function LeaderboardEntryRow({ entry, isCurrentUser, sortBy }) {
...
export function TopThreePodium({ entries, sortBy, currentUserId }) {
      ...
      {ranks.map((item, idx) => {
        ...
        return (
          ...
          >
            <div className="mb-2">
              <Avatar
                className={`h-16 w-16 border-4 ${item.color} bg-background`}
              >
            ...
            <span className="mt-1 font-bold">
              {value.toLocaleString()} {sortBy === "xp" ? "XP" : "Pts"}
            </span>

            <div
              className={`mt-2 flex w-full items-center justify-center rounded-t-lg ${item.color} py-2`}
            >
