...
import {
  LeaderboardEntryRow,
  TopThreePodium,
} from "@/features/leaderboard/ui/LeaderboardEntry";
...
} from "@/features/leaderboard/ui/LeaderboardRewardsBanner";
...
export default function LeaderboardPage() {
  ...
  const { entries, myPosition, loading, currentUserId } = useLeaderboard({
    category,
    sortBy,
  });
  const { user } = useUser();
  ...
  const fetchRewardPreview = async (cat) => {
    setLoadingRewards(true);
    try {
      const configKey =
        cat === "weekly"
          ? "leaderboard_weekly_rewards"
          : "leaderboard_monthly_rewards";
      ...
      if (response.success && response.data) {
        ...
        try {
          ...
          rewards =
            cat === "weekly" ? DEFAULT_WEEKLY_REWARDS : DEFAULT_MONTHLY_REWARDS;
        }
    ...
    <div className="mx-auto w-full max-w-3xl md:p-8">
      ...
      {loading ? (
        ...
        <div className="flex flex-col items-center justify-center py-12 rounded-lg border">
          <p className="text-muted-foreground">
            No rankings yet for this category
          </p>
        </div>
      ) : (
        <>
