...
import type { RewardTier, LeaderboardEntry } from "@/lib/types/api";

interface LeaderboardRewardsBannerProps {
...
function getTimeRemaining(periodEndsAt: string): string {
...
export function LeaderboardRewardsBanner({
  ...
  periodEndsAt,
  currentUserId
}: LeaderboardRewardsBannerProps) {
  const [timeRemaining, setTimeRemaining] = useState(getTimeRemaining(periodEndsAt));
    ...
    <Card className="mb-6 border-amber-200 bg-gradient-to-r from-amber-50 to-orange-50 dark:from-amber-950/20 dark:to-orange-950/20 dark:border-amber-800/30">
      <CardHeader className="pb-2">
        <div className="flex items-center justify-between">
          ...
          <Badge variant="outline" className="gap-1 bg-amber-100 text-amber-700 dark:bg-amber-900/30 dark:text-amber-400">
        ...
        {currentUserInTop && currentUserReward && (
          <div className="mt-4 rounded-lg border border-primary/20 bg-primary/5 p-3 dark:bg-primary/10">
            <p className="text-sm font-medium">
              ...
              {getRankEmoji(currentUserEntry!.rank)} {currentUserEntry!.rank === 1 ? "1st" : currentUserEntry!.rank === 2 ? "2nd" : "3rd"} Place
              {!isPeriodEnded && (
                <>
                  {" "}
                  <span className="text-muted-foreground">
                    (Potential reward: {currentUserReward.xp} XP + {currentUserReward.coins} coins)
                  </span>
                </>
              )}
            </p>
