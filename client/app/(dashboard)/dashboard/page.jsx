...
export default function DashboardOverview() {
    ...
    const fetchData = async () => {
      try {
        ...
        if (userData.success && userData.data) {
          ...
          username =
            userData.data.firstName || userData.data.username || "User";
          const roles = userData.data.roles || [];
          roleNames = roles.flatMap((row) => {
        ...
        const transformedLeaderboard = dashboardData?.leaderboard
          ? transformLeaderboard(dashboardData.leaderboard)
          : [];
  ...
  const resolvedLevel =
    data.dashboardData?.stats?.level ??
    data.dashboardData?.user?.xp?.level ??
    1;
  const resolvedXp =
    data.dashboardData?.stats?.xp ?? data.dashboardData?.user?.xp?.xp ?? 0;
