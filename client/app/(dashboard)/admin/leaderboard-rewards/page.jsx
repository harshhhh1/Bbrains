...
export default function LeaderboardRewardsAdminPage() {
  ...
  const fetchRewards = async () => {
    setLoading(true);
    try {
      const weeklyRes = await api.get("/config/leaderboard_weekly_rewards");
      const monthlyRes = await api.get("/config/leaderboard_monthly_rewards");
  ...
  const saveRewards = async (category) => {
  ...
  const updateReward = (category, rank, field, value) => {
      ...
      <Card className="mt-6">
        ...
        <CardContent>
          <ul className="list-inside list-disc space-y-2 text-sm text-muted-foreground">
            <li>
              Rewards are automatically distributed via cron jobs at the end of
              each period
            </li>
            <li>Weekly rewards are distributed every Sunday at 23:59</li>
            <li>
              Monthly rewards are distributed on the 1st of each month at 00:00
            </li>
            <li>Top 3 performers in each college receive rewards</li>
            <li>Users tied at any rank both receive the same reward amount</li>
            <li>
              Admins can modify reward amounts using this page at any time
            </li>
          </ul>
        </CardContent>
      </Card>
