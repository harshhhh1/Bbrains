...
export default function AchievementsPage() {
  ...
  const [form, setForm] = useState(emptyAchForm);
  ...
  const load = useCallback(async () => {
    try {
      ...
      const res = await c.get("/achievements");
  ...
  function openEdit(a) {
    ...
    setForm({
      ...
      requiredXp: String(a.requiredXp),
      category: a.category ?? "",
      icon: a.icon ?? "",
    });
    setModalOpen(true);
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      if (editing) {
        const r = await c.put(`/achievements/${editing.id}`, payload);
        setAchievements((prev) =>
          prev.map((a) => (a.id === editing.id ? r.data.data : a)),
        );
      } else {
        const r = await c.post("/achievements", payload);
  ...
  return (
    <div className="space-y-4">
      ...
      <DataTable
        ...
        columns={[
          ...
          },
          {
            key: "requiredXp",
            label: "Required XP",
            render: (r) => String(r.requiredXp),
          },
          {
            key: "description",
            label: "Description",
            render: (r) => (
              <span className="line-clamp-1 max-w-xs text-muted-foreground text-xs">
                {r.description ?? "—"}
              </span>
            ),
          },
      ...
      >
        ...
        <div className="grid grid-cols-2 gap-3">
          <FormInput
            ...
            onChange={(e) =>
              setForm((f) => ({ ...f, requiredXp: e.target.value }))
            }
          />
          <FormInput
            label="Category"
      ...
      <ConfirmDialog
        ...
        description={`Delete achievement "${deleteTarget?.name}"?`}
      />
    </div>
  );
}
