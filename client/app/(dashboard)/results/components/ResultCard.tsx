...
import type { StudentAssessmentResult } from "@/services/api/client";
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
          <div className="min-w-0 space-y-3">
            <div className="flex flex-wrap items-center gap-2">
              <Badge variant="outline" className="font-bold tracking-widest uppercase text-[9px] px-2 py-0">
          ...
          <div className="rounded-2xl border border-border/60 bg-background/50 backdrop-blur-sm p-4 text-left shadow-inner sm:min-w-[160px] sm:text-right flex flex-col justify-center">
            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground mb-1">Performance</p>
            <div className="flex items-baseline gap-1 sm:justify-end">
               <span className="text-2xl font-black text-foreground tabular-nums">{result.marksObtained}</span>
               <span className="text-xs font-bold text-muted-foreground opacity-40">/ {result.assessment.totalMarks}</span>
            </div>
            <p className={cn(
              "text-sm font-black mt-1",
              percentage >= 75 ? "text-emerald-500" : percentage >= 40 ? "text-primary" : "text-destructive"
            )}>
              {percentage}% Result
            </p>
          </div>
