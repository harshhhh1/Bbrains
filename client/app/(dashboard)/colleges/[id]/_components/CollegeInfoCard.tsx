...
import { Building2, Hash, Mail, MapPin, Edit2, Check, X, Loader2 } from "lucide-react";
...
export function CollegeInfoCard({ college, onUpdate }: CollegeInfoCardProps) {
        ...
        {!isEditing ? (
          ...
          <div className="flex items-center gap-1">
            <Button variant="ghost" size="icon" onClick={handleSave} disabled={loading}>
              {loading ? <Loader2 className="size-4 animate-spin" /> : <Check className="size-4 text-emerald-600" />}
            </Button>
              ...
              <div>
                {college.address ? (
                  <div className="text-sm space-y-1">
                    <p className="font-medium">{college.address.addressLine1}</p>
                    {college.address.addressLine2 && <p>{college.address.addressLine2}</p>}
